...
from jaxpm.painting import cic_paint, compensate_cic, cic_paint_dx
...
def make_full_field_model(field_size,
                          field_npix,
                          box_shape,
                          box_size,
                          halo_size=0,
    ...
    def forward_model(cosmo, nz_shear, initial_conditions):
        ...
        # Create particles
        local_mesh_shape = (*get_local_shape(initial_conditions.shape,
                                             sharding), 3)
        ...
        cosmo = jc.Cosmology(Omega_c=cosmo.Omega_c,
                             sigma8=cosmo.sigma8,
        ...
        xgrid, ygrid = np.meshgrid(
            np.linspace(0, field_size, box_shape[0],
                        endpoint=False),  # range of X coordinates
            np.linspace(0, field_size, box_shape[1],
                        endpoint=False))  # range of Y coordinates
        coords = jnp.array(
            (np.stack([xgrid, ygrid], axis=0) * u.deg).to(u.rad))

        # Generate convergence maps by integrating over nz and source planes
        convergence_maps = [
            simps(lambda z: nz(z).reshape([-1, 1, 1]) * convergence_Born(
                cosmo, lightcone, r, a, dx, dz, coords, z),
                  0.01,
                  3.0,
                  N=32) for nz in nz_shear
        ]

        # Reshape the maps to desired resoluton
        convergence_maps = [
            kmap.reshape([
                field_npix, box_shape[0] // field_npix, field_npix,
...
def full_field_probmodel(config):
    ...
    convergence_maps, _ = forward_model(cosmo, config.nz_shear,
                                        initial_conditions)
